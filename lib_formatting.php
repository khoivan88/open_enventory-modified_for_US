...
function toDateTime($str,$seconds=false) {
	...
	// return date_format($result, getISODateTimePattern($seconds));   // Khoi's note: yyyy-mm-dd style
}
...
function toDate($str) {
	...
	// return date_format($result, getISODateTimePattern());
...
// Khoi: return ISO 8601 standard format
function getISODateTimePattern($seconds=true) {
	if ($seconds) {
		return 'c';    // ISO 8601 such as:	2004-02-12T15:19:21+00:00
	} else {
		return 'Y-m-d\TH:iP';    // ISO 8601 without second such as:	2004-02-12T15:19+00:00
	}
}
...
function getTimestampFromDate($date) {    //Khoi's note: mainly used by import function
	if (isset($date)) {
		$result=array();
		if (preg_match("/^(\d{1,2})\.(\d{1,2})\.(\d{2,4})/",$date,$result)) {   // TT.MM.JJJJ: German date style
		if ($result) {
...
// Khoi: similar to getRange() but account for text infront
function getPrice($text) { // return low, high, text after
	$expr_range="/(?ims)\(?([A-Z]{3}|[A-Z]?[\$۴])?\s?(\-?[\d\.,]+)\)?\s*[\-]\s*\(?(\-?[\d\.,]+)\)?(.*)/";
	$expr_sing="/(?ims)\(?([A-Z]{3}|[A-Z]?[\$۴])?\s?(\-?[\d\.,]+)\)?(.*)/";
	if (preg_match($expr_range,$text,$results)) {
		return array(fixNumber($results[2]),fixNumber($results[3]),trim($results[1]).trim($results[4]));
	}
	if (preg_match($expr_sing,$text,$results)) {
		return array("",fixNumber($results[2]),trim($results[1]).trim($results[3]));
	}
	return array("","","");
}
