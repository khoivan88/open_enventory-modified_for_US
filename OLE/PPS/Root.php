    ...
    */
    function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt)
    {
        ...
        fwrite($FILE,
                  ...
                  . pk("w", $iBBcnt+$iSBDcnt) //ROOT START
                  //~ . pack("V", 0)
                  . pk("w", 0)
                  //~ . pack("V", 0x1000)
                  . pk("w", 0x1000)
                  //~ . pack("V", 0)                  //Small Block Depot
                  . pk("w", 0)                  //Small Block Depot
                  //~ . pack("V", 1)
                  . pk("w", 1)
          );
        ...
        if ($iBdCnt < $i1stBdL) {
            fwrite($FILE,
                      ...
                      pk("w", -2).      // Extra BDList Start
                      //~ pack("V", 0)        // Extra BDList Count
                      pk("w", 0)        // Extra BDList Count
                  );
        } else {
            //~ fwrite($FILE, pack("V", $iAll+$iBdCnt) . pack("V", $iBdExL));
            fwrite($FILE, pk("w", $iAll+$iBdCnt) . pk("w", $iBdExL));
        }

        // BDList
        for ($i = 0; $i < $i1stBdL && $i < $iBdCnt; $i++) {
            //~ fwrite($FILE, pack("V", $iAll+$i));
            fwrite($FILE, pk("w", $iAll+$i));
        }
        if ($i < $i1stBdL) {
    ...
    */
    function _saveBbd($iSbdSize, $iBsize, $iPpsCnt) 
    {
        ...
        for ($i = 0; $i < ($iPpsCnt - 1); $i++) {
            ...
            fwrite($FILE, pk("w", $i+$iSbdSize+$iBsize+1));
        }
        //~ fwrite($FILE, pack("V", -2));
        fwrite($FILE, pk("w", -2));
        ...
        for ($i = 0; $i < $iBdCnt; $i++) {
            ...
            fwrite($FILE, pk("w", 0xFFFFFFFD));
        }
        // Set for ExtraBDList
        for ($i = 0; $i < $iBdExL; $i++) {
            //~ fwrite($FILE, pack("V", 0xFFFFFFFC));
            fwrite($FILE, pk("w", 0xFFFFFFFC));
        }
        // Adjust for Block
        ...
        if ($iBdCnt > $i1stBdL) {
            ...
            for ($i = $i1stBdL;$i < $iBdCnt; $i++, $iN++) {
                if ($iN >= ($iBbCnt - 1)) {
                    ...
                    fwrite($FILE, pk("w", $iAll+$iBdCnt+$iNb));
                }
                //~ fwrite($FILE, pack("V", $iBsize+$iSbdSize+$iPpsCnt+$i));
                fwrite($FILE, pk("w", $iBsize+$iSbdSize+$iPpsCnt+$i));
            }
