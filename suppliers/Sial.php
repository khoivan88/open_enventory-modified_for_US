...
$GLOBALS["suppliers"][$GLOBALS["code"]]=new class extends Supplier {
	...
	public function procDetail(& $response,$catNo="") {
		...
		$cookies=array_merge($cookies,$this->country_cookies);
		...
		for ($b=0;$b<count($specialLines);$b++) {
			$this->handleCells($result,array_slice($specialLines[$b],1));
		}
		...
		$my_http_options["referer"]=$this->getDetailPageURL($catNo);
	...
	public function handleCells(& $result,$cells) {
		...
		elseif (strpos($text,"EC Number")!==FALSE) {
			if (!isEmptyStr($next_text)) {
				$result["molecule_property"][]=array("class" => "EG_No", "source" => $this->code, "conditions" => $next_text);
			}
		}
		...
		elseif (strpos($text,"RIDADR")!==FALSE) {
			$result["molecule_property"][]=array("class" => "adr", "source" => $this->code, "conditions" => $next_text);
			// get packing group, after last comma
			$lastSlash=strrpos($next_text,"/");
			if ($lastSlash!==FALSE) {
				$result["molecule_property"][]=array("class" => "packing_group", "source" => $this->code, "conditions" => getNumber(substr($next_text,$lastSlash+1)), );
			}
		}
		elseif (strpos($text,"Flash Point(C)")!==FALSE) {
			if (!isEmptyStr($next_text)) {
				$result["molecule_property"][]=array("class" => "FP", "source" => $this->code, "value_high" => $next_text+0.0, "unit" => "°C");
			}
		}
		...
		elseif (strpos($text,"expl. lim.")!==FALSE) { // nur obere Grenze
			if (!isEmptyStr($next_text)) {
				$result["molecule_property"][]=array("class" => "Ex_limits", "source" => $this->code, "value_high" => $next_text+0.0, "unit" => "Vol.-%");
			}
		}
    }
