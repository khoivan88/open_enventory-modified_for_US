...
/**
 * Khoi: modify to add temporary new form action
 * @param {string} id : form id
 * @param {string} action : the temporary new form action
 */
function submitForm(id, action=undefined) {
    var obj = $(id);
    if (obj) {
        if (action) {
            // Set HTML5 data-attr for the original action attr
            obj.setAttribute('data-original-action', obj.action)
            // Set the form action attr to a new one
            obj.action = action;
        } else {
            // Reset form action to the original form action attr or using the action attr in case 'data-original-action' has not been set
            obj.action = obj.getAttribute('data-original-action') || obj.action
        }
        // console.log(`original form action is: ${obj.action}`)
        obj.submit();
    }
}
