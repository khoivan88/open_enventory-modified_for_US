...
function loadCaptcha(btn) {
	hideObj(btn);
	setFrameURL("comm", "chooseAsync.php?desired_action=loadSciflectionCaptcha");
	setTimeout(function (){showObj(btn);},3000); // show again after 3 sec to prevent people from blocking themselves
}

function makeUsername(organisation_name) {
	if (!organisation_name) {
		return "";
	}
	return organisation_name.trim().toLowerCase().replace(/\W/g,"_");
}

function updateResults(int_name,search) {
	var url = "chooseAsync.php?" + getSelfRef(["~script~", "table", "cached_query", "query"]) + "&int_name="+int_name+"&desired_action=searchAssignableEntries&search=" + encodeURIComponent(search);
	setFrameURL("comm", url);
}

function addThisToDataPublication(data_publication_id) {
	var add_crit;
	if (table=="reaction") {
		add_crit="reaction.reaction_id";
	} else if (table=="project") {
		add_crit="reaction.project_id";
	} else if (table=="lab_journal") {
		add_crit="reaction.lab_journal_id";
	}
	open("edit.php?table=data_publication&"+(data_publication_id ? "dbs=-1&query=%3C0%3E&crit0=publication_status&op0=eq&val0=1&edit=true&db_id=-1&pk=" + data_publication_id : "desired_action=new") + "&" + getSelfRef(["~script~", "table", "cached_query", "query"]) + "&add_crit=" + add_crit + "&add_pk=" + a_pk);
}

function submitDataPublication() {
	var currentEntry=a(dataCache,a_db_id,a_pk);
	if (a_db_id=="-1" && confirm(s("confirmSubmit1")+ac(currentEntry,"publication_reaction")+s("confirmSubmit2")+ac(currentEntry,"publication_analytical_data")+s("confirmSubmit3")+a(currentEntry,"publication_license")+s("confirmSubmit4")+a(currentEntry,"db_beauty_name")+s("confirmSubmit5")+a(currentEntry,"host")+s("confirmSubmit6"))) {
		setFrameURL("edit", "editAsync.php?" + getSelfRef(["~script~", "cached_query", "query"]) + "&refresh=true&desired_action=confirm&db_id="+a_db_id+"&pk=" + a_pk);
	}
}
...
function downloadRxnPDF() { // no longer used
	var url="getPDF.php?table=reaction&db_id="+a_db_id+"&pk="+a_pk;
	window.open(url);
}

function showPDFMenu(show) {
	visibleObj("pdfMenu",show);
}

function startPDF() {
	var url="getPDF.php?"+keepParams(["table","view_options"]);
//	if (getChecked("pdf_selection")) {
//		url+="&per_page=-1&query=&selected_only=1&list_op=2&ref_cache_id="+a(self_ref,"cached_query");
//	}
//	else {
		if (getChecked("pdf_current")) {
			url+="&db_id="+a_db_id+"&pk="+a_pk;
		}
		else {
			// accept only datasets for own DB (-1)
			var pks=[];
			if (getChecked("pdf_range")) {
				// transform into list of pks
				var datasetNumbers=splitDatasetRange(getInputValue("pdf_range_input"));
				for (var i=0,iMax=datasetNumbers.length;i<iMax;i++) {
					var idx=datasetNumbers[i];
					if (dbIdx[idx]["db_id"]==-1) {
						pks.push(dbIdx[idx]["pk"]);
					}
				}
			} else {
				for (var i=0,iMax=dbIdx.length;i<iMax;i++) {
					if (dbIdx[i]["db_id"]==-1) {
						pks.push(dbIdx[i]["pk"]);
					}
				}
			}
			url+="&db_id=-1&pk="+pks.join(",");
		}
//	}
	window.open(url);
	showPDFMenu(false);
...
function updatePermissions() {
	...
	if (value!=="") {
		setControlValues({
			"permissions": value,
			"permissions_general": value,
			"permissions_chemical": value,
			"permissions_lab_journal": value,
			"permissions_order": value,
			"permissions_borrow_external": value, // Khoi: added option for Terminal function to let users from other groups borrow the chemical
		}, true);
	}
}
...
function initVersionsList() {
	...
	for (var b=0,max=versions.length;b<max;b++) {
		...
		// Khoi: use yyyy-mm-dd date format instead
		text = (max - b) + ": " + toInternationalDate(a(versions, b, "version_when")) + " (" + a(versions, b, "version_by") + ") " + versions[b]["version_comment"];
...
function refreshLiteratureImgId(list_int_name,UID,int_name) { // leave a_timestamp untouched in general
	...
	SILsetValueUID(list_int_name,UID,undefined,undefined,"literature_graphics_blob",undefined,{db_id:a_db_id,literature_id:SILgetValue(list_int_name,UID,"literature_id")});
	a_timestamp=temp;
}
...
function get_reference_link(ref_table,db_id,pk) {
	return "<a href=\""+get_reference_url(ref_table,db_id,pk)+"\" class=\"imgButtonSm\" target=\"_blank\"><img src=\"lib/details_sm.png\" border=\"0\""+getTooltip("details")+"></a>";
}
...
function updateButtons() {
	...
	if (editMode) {
		...
		visibleObj("buttons_ro_always",ro_other && (a_db_id==-1)); // inline
...
function del(no_confirm) {
	...
	if (table!="reaction") { //  && dispose_instead_of_delete!=true
		setTimeout(function () { // must finish calling editAsync.php
		// aus cache löschen
		deleteDatasetFromCache(actIdx);
		},10);
	}
...
function update_mass_calc(src) {
	...
	switch (getUnitType(amount_unit)) {
		case "v":
		density_20=def1(def(getInputValue("chemical_storage_density_20"),getInputValue("density_20")));
		if (density_20<=0) {
			density_20=1;
		}
		case "":
