...
function today() {
	...
	return getSQLDate(getDeltaDate(Number(new Date()) ) );
}
...
// Khoi: to get SQL style date (yyyy-mm-dd)
function getSQLDate(dateObj) {
...
// // Khoi: rewrite toSQLDate to take into account different kind of date
...
//     var found=0;
//     /* Khoi: for date in German format (dd.mm.yyyy)---------------------------------------------------------------------------*/
//     var gerDateFmt=/^(\d\d?)\.(\d\d?)\.(\d{2,4})$/;  // TT.MM.JJ oder TT.MM.JJJJ
// 	if (gerDateFmt.exec(date)) {
// 		var year=parseInt(Number(RegExp.$3)),month=RegExp.$2,day=RegExp.$1;
// 		found=1;
// 	}
// 	if (found==0) {
// 		var gerDateTimeFmt1=/^(\d\d?)\.(\d\d?)\.(\d{2,4}) (\d\d?)[\.:](\d\d)[\.:](\d\d)$/;
// 		if (gerDateTimeFmt1.exec(date)) {
// 			var year=parseInt(Number(RegExp.$3)),month=RegExp.$2,day=RegExp.$1,hour=RegExp.$4,minute=RegExp.$5,second=RegExp.$6;
// 			found=2;
// 		}
// 	}
...
//     /* Khoi: for date in SQL format (yyyy-mm-dd)---------------------------------------------------------------------------*/
//     var SQLDateFmt=/^(\d{4})-(\d\d)-(\d\d)$/;
// 	if (SQLDateFmt.exec(date)) {
// 		var year=parseInt(Number(RegExp.$1)),month=RegExp.$2,day=RegExp.$3;
// 		found=1;
// 	}
// 	if (found==0) {
//         var SQLDateTimeFmt1=/^(\d{4})-(\d\d)-(\d\d) (\d\d):(\d\d):(\d\d)$/;
// 		if (SQLDateTimeFmt1.exec(date)) {
// 			var year=parseInt(Number(RegExp.$1)),month=RegExp.$2,day=RegExp.$3,hour=RegExp.$4,minute=RegExp.$5,second=RegExp.$6;
// 			found=2;
// 		}
// 	}
// 	if (found==0) {
//         var SQLDateTimeFmt2=/^(\d{4})-(\d\d)-(\d\d) (\d\d):(\d\d)$/;
// 		if (SQLDateTimeFmt2.exec(date)) {
//             var year=parseInt(Number(RegExp.$1)),month=RegExp.$2,day=RegExp.$3,hour=RegExp.$4,minute=RegExp.$5,second="00";
// 			found=2;
// 		}
//     }
//     /* -----------------------------------------------------------------------------------------------------------------------*/

//     /* Khoi: for date in American format (mm/dd/yyyy)---------------------------------------------------------------------------*/
//     var americanDateFmt=/^(\d\d?)\/(\d\d?)\/(\d{2,4})$/;
// 	if (americanDateFmt.exec(date)) {
// 		var year=parseInt(Number(RegExp.$3)),month=RegExp.$1,day=RegExp.$2;
// 		found=1;
// 	}
// 	if (found==0) {
// 		var americanDateTimeFmt1=/^(\d\d?)\/(\d\d?)\/(\d{2,4}) (\d\d?)[\.:](\d\d)[\.:](\d\d)$/;
// 		if (americanDateTimeFmt1.exec(date)) {
// 			var year=parseInt(Number(RegExp.$3)),month=RegExp.$1,day=RegExp.$2,hour=RegExp.$4,minute=RegExp.$5,second=RegExp.$6;
// 			found=2;
// 		}
// 	}
// 	if (found==0) {
//         var americanDateTimeFmt2=/^(\d\d?)\/(\d\d?)\/(\d{2,4}) (\d\d?)[\.:](\d\d)$/;
// 		if (americanDateTimeFmt2.exec(date)) {
//             var year=parseInt(Number(RegExp.$3)),month=RegExp.$1,day=RegExp.$2,hour=RegExp.$4,minute=RegExp.$5,second="00";
// 			found=2;
// 		}
//     }
//     /* -----------------------------------------------------------------------------------------------------------------------*/
...
function toSQLDate(localDate) {
	...
	var result=moment(localDate, jsDateFormat+" "+jsTimeFormat, false);
	if (result.isValid()) {
		return result.format(sqlDateFormat+" "+sqlTimeFormat);
	}
	result=moment(localDate, jsDateFormat, false);
	if (result.isValid()) {
		return result.format(sqlDateFormat);
	}
	return invalidSQLDate;
}
...
function toInternationalDate(SQLDate) {
	if (!SQLDate || SQLDate==invalidSQLDate || SQLDate==invalidSQLDateTime) {
		return "";
	}
	var result=moment(SQLDate, sqlDateFormat+" "+sqlTimeFormat, false);
	if (result.isValid()) {
		return result.format(jsDateFormat+" "+jsTimeFormat);
	}
	result=moment(SQLDate, sqlDateFormat, false);
	if (result.isValid()) {
		return result.format(jsDateFormat);
	}
	return "";
}
...
function splitDatasetRange(datasetRange,shift_down) {
	...
	if (shift_down==undefined) {
		shift_down=1; // 1-based (human) to 0-based (array)
	}
	...
	for (var b=0,max=fragments.length;b<max;b++) {
		...
		for (var c=Math.max(1,Math.min(range[0],range[1])),max=Math.min(Math.max(range[0],range[1]),maxDataset);c<=max;c++) { // avoid endless loops from a-b where a>b
			retval.push(c-shift_down); // add expanded indexes to return value
		}
	}
...
function evalNum(num) {
	...
	num=num.replace(/x/g,"*"); // allow multiplication using x
	num=num.replace(/[a-df-zA-DF-Z\s\"\']/g,""); // prevent function calls, keep e's for exponential notation
	num=num.replace(/e+\(/g,"("); // remove potential e( function calls
	num=num.replace(/,/g,".");
	num=num.replace(/%/g,"*0.01");
	num=eval(num);
	return num;
}
