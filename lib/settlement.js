...
function getSettlementPrintout() {
	var iHTML="",chem_order=dataCache[a_db_id][a_pk]["accepted_order"],rent=dataCache[a_db_id][a_pk]["rent"],net_total=0,vat_sum=0,currency=getCacheValue("currency"),from_date=toInternationalDate(getCacheValue("from_date")),to_date=toInternationalDate(getCacheValue("to_date")),name,kst,acc_no,supplier,firstDone;
