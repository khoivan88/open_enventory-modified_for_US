...
function loadDataFromSciflection(url) {
	if (url) {
		var url = "chooseAsync.php?" + getSelfRef(["~script~", "cached_query", "query"]) + "&desired_action=loadExpFromUrl&url=" + encodeURIComponent(url);
		setFrameURL("comm", url);
	} else {
		open("https://sciflection.com/easyPortal.uc?useCase=srch&table=ElnReaction");
	}
}
