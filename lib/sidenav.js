...
function switchSideframe(big) {
	...
	if (big) {
		...
		fs_obj.cols="0,*,0"; // 3rd row ignored if not there
		f_obj.scrolling="no";
	}
	...
	//Khoi: don't need these anymore. EventListener in topnav.php takes care of this.
	// visibleObj("collapse",big);
	// visibleObj("expand",!big);
	// visibleObj("bg_layer",!big);

	// var expandIcon=top.$("topnav").contentDocument.getElementById("expand-icon");
	// visibleObj(expandIcon,!big);
}
...
function addCrit(after_num) {
	...
	makeCombobox("crit"+next_num);
	setSelectedIndex("op"+next_num,0);
	...
	// Khoi: activate Select2() for new select element:
	jQuery(`#crit${next_num}`).select2();
...
function makeCombobox(selectName) {
	jQuery("select[name=\""+selectName+"\"]").scombobox({
		...
		callback: {
			func: function () {
				jQuery(this).children(".scombobox-display").attr("autocomplete", "off");
				jQuery(this).attr("id", selectName+"-combo").css({"max-width": "180px","display": "inline-block"}).children("select").attr("id", selectName); // compat
				jQuery(this).children("input").focus(function () {jQuery(this).select();}); // select all onFocus
			}
		}
	});
}
...
function updateSelects() {
	...
	if (type==currentType) { // no change of type
        ...
        // Khoi: to automatically change size of sidenav upon choosing Structure search
        if (type=="structure") {expandWidth();}
        else {resetWidth();}
...
// Khoi: to automatically change size of sidenav upon choosing Structure search
var frameset=top.$("sideframe");
var original_cols=top.$("sideframe").getAttribute("cols");
function expandWidth() {
    frameset.setAttribute("cols","350,*");
}
function resetWidth() {
    frameset.setAttribute("cols",original_cols);
}
...
function updateDate() {
	...
	case 1: // today
		...
		new_value=getSQLDate(getDeltaDate(now));
	break;
	case 2: // yesterday
		// new_value=getGerDate(getDeltaDate(now,-1));
		new_value=getSQLDate(getDeltaDate(now,-1));
	break;
	case 3: // this week
		// new_value=getGerDate(getDeltaDate(now,-6))+"-"+getGerDate(getDeltaDate(now));
		new_value=getSQLDate(getDeltaDate(now,-6))+"-"+getSQLDate(getDeltaDate(now));
	break;
	case 4: // this month
		// new_value=getGerDate(getDeltaDate(now,-29))+"-"+getGerDate(getDeltaDate(now));
		new_value=getSQLDate(getDeltaDate(now,-29))+"-"+getSQLDate(getDeltaDate(now));
	break;
