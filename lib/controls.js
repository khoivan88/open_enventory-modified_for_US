...
function setControl(int_name,values,init,noOverwrite) { // init teilt setFunction für pk mit, daß initialisierung im gang ist, vermeidet endlosschleife, noOverwrite for readExt
	...
	case "input":
		...
		break;
		case "date":
			value=toInternationalDate(value);
		break;
		case "password":
			value="";
		break;
	...
	case "pk_select":
		if (!a(contArr,"multiMode")) {
			if (a(contArr,"dynamic")) {
				...
				if (a(contArr,READ_ONLY)) { // single (fake) value
...
function updateControl(int_name) {
	var isEmpty=false,elementType=getControlElementType(int_name),contArr=a(controls,int_name),thisReadOnly=a(contArr,READ_ONLY),visible=a(contArr,VISIBLE),thisLocked=a(contArr,LOCKED),rw=$("rw_"+int_name),ro=$("ro_"+int_name);
...
function readOnlyControl(int_name,thisReadOnly) {
	changeControlProperty(int_name,thisReadOnly,READ_ONLY,DEFAULTREADONLY);
}
