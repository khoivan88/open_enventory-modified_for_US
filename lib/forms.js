...
function readOnlyForm(int_name,thisReadOnly) {
	changeFormProperty(int_name,thisReadOnly,READ_ONLY,DEFAULTREADONLY);
}

function changeFormsProperty(thisValue,propertyName,defaultBehaviour) { // true, READ_ONLY, DEFAULTREADONLY
	var defaultValue;
	for (var key in formulare) {
...
function readOnlyForms(thisReadOnly) {
	changeFormsProperty(thisReadOnly,READ_ONLY,DEFAULTREADONLY);
	updateMolecules();
...
function checkSubmitForm(int_name) {
	if (a(formulare,int_name,READ_ONLY) || a(formulare,int_name,LOCKED) || !a(formulare,int_name,VISIBLE)) { // dont check readOnly or locked Forms
		return true;
	}
	for (var b=0,max=formulare[int_name]["controls"].length;b<max;b++) {
