...
$GLOBALS["device_driver"]="bruker_xwin";
$GLOBALS["analytics"][ $GLOBALS["type_code"] ][ $GLOBALS["device_driver"] ]=array(
"requiredFiles" => array(".aqs",".fqs",".1r"),
"optionalFiles" => array(".1i",".int"));
/*
 * Reads and converts a bruker_xwin file to sketchable graphdata
 */

class bruker_xwin extends converter {
	...
	function __construct($file_content, $doConstruction) {
		if($doConstruction==true) {
			...
			$this->data = $file_content['.1r'][$this->fileNumber];	// puts all the data into a string
			$this->report[0] = $file_content['.fqs'][$this->fileNumber];
			$this->report[1] = $file_content['.aqs'][$this->fileNumber];
	...
	/*
	 * converts a bruker_xwin file into sketchable graphData
	 */
	public function convertFileToGraphData() {
		...
		for ($a=0; $a<$blocks_count; $a++) {
			...
			$pointsAsXY[$a]['y']= $this->readData($blocks[$a], 'f', strlen($blocks[$a]), 0);
			if($pointsAsXY[$a]['y']<0) {
				$pointsAsXY[$a]['y']=0;
			}
		...
		$this->graphData['drawingStyle'] = 2; // set drawingStyle to y axis right
