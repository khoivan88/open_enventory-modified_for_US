...
function showPersonEditForm($paramHash) { // ergänzen: Kostenstelle, Kontonummer[, Kostenlimit, Hostname]
    ...
    if ($editMode) {
		$paramHash["change"][READ_ONLY]=
            ...
            'showControl("permissions_borrow_external",!is_self_rw); '.
    ...
    $retval=getFormElements($paramHash,array(
		...
		array("item" => "input", "int_name" => "title", "size" => 5,"maxlength" => 20, "showAlways" => true, "text" => "", "placeholder" => s("title")), 
        array("item" => "text", SPLITMODE => true, "text" => " "),
		array("item" => "input", SPLITMODE => true, "int_name" => "first_name","size" => 20,"maxlength" => 50, "placeholder" => s("first_name")), 
        array("item" => "text", SPLITMODE => true, "text" => " "),
		array("item" => "input", SPLITMODE => true, "int_name" => "last_name", "size" => 20,"maxlength" => 50, "placeholder" => s("last_name")), 
        ...
        array("item" => "checkset", "int_name" => "permissions_borrow_external", "loadBlind" => true, "int_names" => getIntNames($permissions_groups,4), "texts" => getTexts($permissions_groups,$perm_texts,4), "shift" => cumSum($permissions_groups,4), "breakAfter" => 2, "getValue" => "return values[\"permissions\"];", ),
